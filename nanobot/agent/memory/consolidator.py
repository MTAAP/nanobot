...
class MemoryConsolidator:
    ...
    def consolidate(
        ...
        """Consolidate extracted facts into the memory store (namespace by fact_type)."""
        self._last_metrics = ConsolidationMetrics()
        ...
        for fact in facts:
            ...
            result, valid_ids = self._consolidate_single(
                fact.content.strip(), target_ns
            )
            ...
            self._execute_operation(result, target_ns, importance, valid_ids)
        logger.info(
            "consolidation_complete",
            added=self._last_metrics.added,
            updated=self._last_metrics.updated,
            deleted=self._last_metrics.deleted,
            skipped=self._last_metrics.skipped,
        )
        return results
        ...
        try:
            if result.operation in (Operation.UPDATE, Operation.DELETE):
                if result.memory_id and result.memory_id not in valid_ids:
                    logger.warning(
                        f"Invalid memory_id {result.memory_id}, falling back to ADD"
                    )
            ...
            if result.operation == Operation.ADD:
                content = (
                    self._sanitize_storage_content(result.new_content)
                ...
                if not updated:
                    logger.info(
                        f"Memory {result.memory_id} not found, creating new memory instead"
                    )
                    item = self.store.add(
                        content,
                        metadata={"importance": importance},
                ...
                logger.debug(
                    f"Skipped duplicate: {result.new_content[:50] if result.new_content else 'N/A'}..."
                )
