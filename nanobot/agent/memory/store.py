...
class VectorMemoryStore:
    """SQLite-based vector memory store. Single-threaded design for personal assistant use."""
    ...
    def update(
        ...
        priority = max(
            0.0,
            min(1.0, importance * 0.4 + recency * 0.3 + access_score * 0.3),
        )
        self._conn.execute(
            (
                "UPDATE memories "
                "SET content = ?, embedding = ?, metadata = ?, updated_at = ?, priority = ? "
        ...
        if count > self.max_memories:
            ...
            cursor = self._conn.execute(
                (
                    ...
                    "WHERE namespace = ? ORDER BY priority ASC, updated_at ASC LIMIT ?"
                ),
                (namespace, excess),
            )
            ...
            self._conn.commit()
            logger.info(
                f"Pruned {len(ids_to_delete)} old memories from namespace '{namespace}'"
            )
