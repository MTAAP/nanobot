...
Return JSON array with type field: [{{"type": "user", "fact": "...", "importance": "high"}}]
Example: [{{"type": "user", "fact": "User's name is John", "importance": "high"}}]

Facts:"""


LESSON_EXTRACTION_PROMPT = """Analyze this correction sequence and extract a reusable lesson.
...
class ExtractedFactSchema(BaseModel):
    """Pydantic schema for validating LLM-extracted facts (generic/backward compat)."""
...
class MemoryExtractor:
    ...
    def extract(self, messages: list[dict[str, Any]], max_facts: int = 5) -> list[ExtractedFact]:
        ...
        try:
            ...
            for f in facts:
                ...
                self._last_metrics.facts_by_type[t] = (
                    self._last_metrics.facts_by_type.get(t, 0) + 1
                )
            logger.info(
                "extraction_complete",
                facts=len(facts),
                by_type=dict(self._last_metrics.facts_by_type),
            )
            return facts
    ...
    def extract_lessons(
        ...
        """Extract lesson-style facts from explicit user corrections (LLM generalization)."""
        if not messages:
            return []
        ...
        for i, msg in enumerate(window):
            ...
            for j in range(i - 1, -1, -1):
                if window[j].get("role") == "assistant":
                    prev_content = window[j].get("content", "")
                    if isinstance(prev_content, str) and prev_content.strip():
                        previous_answer = prev_content.strip()[:1500]
                    break
            ...
            try:
                generalized = self._llm_extract_lesson(
                ...
                if generalized:
                    ...
                    importance = {"high": 0.9, "medium": 0.7, "low": 0.3}.get(
                        generalized.importance, 0.7
                    )
                    source = "llm_lesson"
            ...
            if lesson_content and lesson_content not in seen:
    ...
    def extract_for_pre_compaction(
        self, messages: list[dict[str, Any]], max_facts: int = 10
    ) -> list[ExtractedFact]:
        """Extract facts from conversation for pre-compaction memory flush (single LLM turn)."""
        if not messages:
            return []
        ...
        try:
            ...
            if not isinstance(data, list):
                return []
            importance_map: dict[str, float] = {"high": 0.9, "medium": 0.7, "low": 0.3}
            out: list[ExtractedFact] = []
            for item in data[:max_facts]:
                if isinstance(item, dict) and item.get("fact"):
                    fact = self._sanitize_fact_content(str(item["fact"]))
                    if not self._is_valid_fact(fact):
                        continue
                    imp = importance_map.get(
                        str(item.get("importance", "medium")).lower(), 0.7
                    )
                    out.append(
                        ExtractedFact(
                            content=fact,
    ...
    def extract_tool_lessons(
        ...
        """Extract tool-specific lessons from tool execution failures in messages."""
        lessons: list[ExtractedFact] = []
        seen: set[str] = set()
        failure_indicators = ("error:", "failed", "exit code:", "exception", "traceback")
        ...
        for msg in messages:
            ...
            # First line of error or first 200 chars
            first_line = content.strip().split("\n")[0][:200]
            lesson_content = (
                f"When using {tool_name}: avoid outcomes that lead to: {first_line}"
            )
            ...
            lessons.append(
                ExtractedFact(
                    ...
                    metadata={"tool_name": tool_name, "error_type": "execution"},
                )
            )
    ...
    def _sanitize_for_prompt(self, text: str) -> str:
        ...
        return text[:MAX_CONVERSATION_CHARS] + "..." if len(text) > MAX_CONVERSATION_CHARS else text

    def _sanitize_fact_content(self, text: str) -> str:
        """Sanitize fact content for storage: strip controls, normalize, length cap."""
        if not text:
            return ""
        text = "".join(c for c in text if unicodedata.category(c) != "Cc")
        text = re.sub(r"\s+", " ", text).strip()
        text = text.replace("<", "&lt;").replace(">", "&gt;")
        return text[:MAX_FACT_CONTENT_LENGTH] if len(text) > MAX_FACT_CONTENT_LENGTH else text

    def _is_valid_fact(self, text: str) -> bool:
        """Return False if fact is empty, too short, too long, or looks like injection."""
    ...
    def _format_conversation(self, messages: list[dict[str, Any]]) -> str:
        """Format messages for the extraction prompt with sanitization and token budget."""
        parts: list[str] = []
        total = 0
    ...
    def _normalize_lesson(self, text: str) -> str:
        """Make lesson third-person and timeless; remove temporal references."""
        if not text or len(text.strip()) < 10:
            return ""
        text = text.strip()
        # Remove temporal references
        for phrase in ["yesterday", "earlier", "that time", "last time", "just now"]:
            text = re.sub(rf"\b{re.escape(phrase)}\b", "", text, flags=re.IGNORECASE)
        text = re.sub(r"\s+", " ", text).strip()
        # Already third-person if from LLM; heuristic may be first-person
            ...
            for item in raw_data[: self.max_facts]:
                try:
                    if not isinstance(item, dict):
                        ...
                        meta = {}
                        if hasattr(validated, "project_name") and getattr(validated, "project_name"):
                            meta["project_name"] = getattr(validated, "project_name")
                        if hasattr(validated, "tool_name") and getattr(validated, "tool_name"):
                            meta["tool_name"] = getattr(validated, "tool_name")

                    sanitized_fact = self._sanitize_fact_content(validated.fact)
                    if not self._is_valid_fact(sanitized_fact):
                        continue
                    extracted.append(
                        ExtractedFact(
                            content=sanitized_fact,
                            importance=importance_map.get(
                                getattr(validated, "importance", "medium"), 0.5
                            ),
                            source="llm",
                            fact_type=fact_type,
                            metadata=meta,
    ...
    def _heuristic_extract(self, messages: list[dict[str, Any]]) -> list[ExtractedFact]:
        """Extract facts using simple heuristics (fallback), with type classification."""
        facts: list[ExtractedFact] = []
        seen: set[str] = set()

        # (indicator, importance, fact_type)
            ...
            for indicator, importance, fact_type in patterns:
                if indicator in content:
                    ...
                    if fact_text and len(fact_text) > 5:
                        ...
                        sanitized = self._sanitize_fact_content(fact)
                        if (
                            sanitized
                            and self._is_valid_fact(sanitized)
                            and sanitized not in seen
                        ):
                            facts.append(
                                ExtractedFact(
                                    content=sanitized,
